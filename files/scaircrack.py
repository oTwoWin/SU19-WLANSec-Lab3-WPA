...
#Recover the SSID from the beacon
ssid        = wpa[0].info
...
# We extract the EAPOL information in the packet
ea = wpa[8][EAPOL]
...
# Recover the MIC in the middle of the packet
mic_to_test = wpa[8].load.encode("hex")[-36:-4]

B           = min(APmac,Clientmac)+max(APmac,Clientmac)+min(ANonce,SNonce)+max(ANonce,SNonce) #used in pseudo-random 
